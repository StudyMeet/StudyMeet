    ...
    return (
        <div className='absolute dark:bg-black-900 bg-gray-900 rounded-md mt-2 min-w-max w-48 right-0 shadow-md'>
            { options.map((option, i) =>
                <div 
                    className={`flex justify-between first:rounded-t-md last:rounded-b-md cursor-pointer px-3 py-1 text-sm font-medium ${colors.get(option.color || 'default')} ${ option.color && colors.get(option.color) }`} 
                    key={i}
                >
